...
const Question = ({question, answer, key}: Props) => {
    ...
    return (
        <div key={key} className={`mx-2 my-2 p-2 bg-slate-700 rounded-xl text-white md:mx-10 lg:mx-28 xl:mx-56 h-auto origin-top transition-all duration-300 relative`}>
            <button className={`flex w-full gap-1 items-center text-left text-white md:font-bold text-xs md:text-lg lg:text-xl px-2 py-1 bg-green-500 rounded-xl`} onClick={() => setRevealed(prev => !prev)}>
